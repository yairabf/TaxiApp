...
int Tcp::acceptClient() {
	...
	descriptorCC = accept(this->socketDescriptor,
...
***********************************************************************/
int Tcp::sendData(string data, int descriptorCC) {
	int data_len = data.length() + 1;
	const char * datas = data.c_str();
	int sent_bytes = send(this->isServer ? descriptorCC
...
***********************************************************************/
int Tcp::reciveData(char* buffer, int size, int descriptorCC) {
	int read_bytes = recv(this->isServer ? descriptorCC
