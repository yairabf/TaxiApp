...
/**
 * theses following tests check a method that operates in this way:
 ...
 * location, using a private method, send the correct driver to the passengers location,
...
TEST_F(TaxiStationTest, answerCallTest) {
    Point p1(0, 0);
    Point p2(5, 5);
    NodeBlock *passengerLocation = new NodeBlock(p1);
    NodeBlock *destination = new NodeBlock(p2);
     ...
     * however, since we have only one driver int the list of "taxi station" then once we run this
     ...
     * of the passenger*/
    EXPECT_EQ(passengerLocation, driver->getLocation()) << "the driver hasnt reached the passenger"<<endl;
    ...
    closeDriver->setLocation(Point(0,1));
